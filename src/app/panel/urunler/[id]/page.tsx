...
export default async function PanelUrunDetayPage({
  ...
  const { data: p } = await supabase
    .from('products')
    .select(
      'id, slug, trendyol_title, trendyol_price, trendyol_stock, trendyol_category, trendyol_barcode, trendyol_images, variant_label, last_synced_at, is_active, override_title, override_description, override_images, badge, is_featured, material_type, gender, note'
    )
    .eq('id', id)
    .maybeSingle()
