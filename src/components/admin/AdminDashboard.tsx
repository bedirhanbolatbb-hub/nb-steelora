...
export default function AdminDashboard({ orders, products, campaigns, syncLogs, reviews: initialReviews }: AdminDashboardProps) {
        ...
        {activeTab === 'reviews' && (
          <div>
            ...
            {localReviews.length === 0 ? (
              <p className="text-text-muted font-body text-sm">Henüz yorum yok.</p>
            ) : (
              <div className="space-y-3">
                  ...
                  <div key={review.id} className="bg-white p-4 flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <div className="flex gap-0.5">
                          {[1, 2, 3, 4, 5].map((s) => (
                            <svg key={s} className={`w-3 h-3 ${s <= review.rating ? 'fill-gold' : 'fill-champagne-mid'}`} viewBox="0 0 24 24">
                              <path d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.562.562 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
                            </svg>
                          ))}
                        </div>
                        <span className="text-[11px] text-text-muted font-body">
                          {review.guest_name || 'Üye'} · {new Date(review.created_at).toLocaleDateString('tr-TR')}
                        </span>
                        {!review.is_approved && (
                          <span className="text-[9px] bg-yellow-100 text-yellow-800 px-1.5 py-0.5 rounded">Bekliyor</span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-2 ml-4 shrink-0">
                      {!review.is_approved && (
                        <button
                          onClick={async () => {
                            await fetch(`/api/admin/campaigns/${review.id}?type=review`, {
                              method: 'PATCH',
                              headers: { 'Content-Type': 'application/json' },
                              body: JSON.stringify({ is_approved: true }),
                            })
                            setLocalReviews((prev) => prev.map((r) => r.id === review.id ? { ...r, is_approved: true } : r))
                          }}
                          className="text-[10px] bg-green-100 text-green-700 px-2 py-1 rounded hover:bg-green-200 transition-colors"
                        >
                          Onayla
                        </button>
                      )}
                      <button
                        onClick={async () => {
                          ...
                          await fetch(`/api/admin/campaigns/${review.id}?type=review`, { method: 'DELETE' })
