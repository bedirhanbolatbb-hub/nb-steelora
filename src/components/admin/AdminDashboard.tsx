...
export default function AdminDashboard({
  ...
  return (
    <div className="min-h-screen bg-bg flex">
      {/* Sidebar */}
      <aside className="w-56 bg-ink-deep text-bg shrink-0 flex flex-col">
        <div className="p-5 border-b border-line/10">
          <h1 className="font-heading text-[18px] font-light tracking-wider">NB Steelora</h1>
          <p className="text-[9px] text-muted uppercase tracking-widest mt-0.5">Admin Panel</p>
        </div>
        <nav className="flex-1 py-4">
          {tabs.map((tab) => (
            <button key={tab.id} onClick={() => tab.id === 'blog' ? handleBlogTabClick() : setActiveTab(tab.id)}
              className={`w-full text-left px-5 py-3 text-[12px] font-body transition-colors flex items-center gap-2 ${activeTab === tab.id ? 'bg-accent/20 text-accent' : 'text-line/70 hover:text-bg hover:bg-line/5'}`}>
              {tab.label}
              {tab.id === 'orders' && pendingCount > 0 && <span className="ml-auto bg-accent text-white text-[9px] w-5 h-5 rounded-full flex items-center justify-center">{pendingCount}</span>}
              {tab.id === 'orderRequests' && orderRequestsPendingCount > 0 && (
                <span className="ml-auto bg-accent text-white text-[9px] min-w-[1.25rem] h-5 px-1 rounded-full flex items-center justify-center">
                  {orderRequestsPendingCount > 99 ? '99+' : orderRequestsPendingCount}
                </span>
              )}
              {tab.id === 'products' && <span className="ml-auto text-[10px] text-muted">{localProducts.length}</span>}
            </button>
          ))}
        </nav>
        <div className="p-4 border-t border-line/10">
          <button onClick={handleLogout} className="w-full text-[11px] text-muted hover:text-accent transition-colors font-body uppercase tracking-wider">Çıkış Yap</button>
        </div>
      </aside>
      ...
      <main className="flex-1 p-6 overflow-auto">
        ...
        {activeTab === 'orders' && (
          <div>
            <h2 className="font-heading text-[24px] text-ink mb-6">Siparişler</h2>
            {orders.length === 0 ? <p className="text-muted font-body text-sm">Henüz sipariş yok.</p> : (
              <div className="bg-white overflow-x-auto">
                <table className="w-full text-[12px] font-body">
                  <thead><tr className="border-b border-line text-muted uppercase tracking-wider">
                  ...
                  <tbody>{orders.map((order) => {
                    ...
                    return (
                    <tr key={order.id} className="border-b border-line/30 hover:bg-bg/50">
                      ...
                      <td className="px-4 py-3 text-right text-accent font-medium">{formatPrice(order.total)}</td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="flex flex-col items-center gap-1">
                          <span className="text-[9px] text-muted leading-tight max-w-[140px]">
                          ...
                          <select
                            ...
                            className="text-[11px] border border-line px-2 py-1 rounded bg-white focus:outline-none"
                          >
                            ...
                            <div className="mt-1 flex flex-col items-center gap-1 w-full max-w-[200px]">
                              {!showTrackingPanel ? (
                                <button
                                  ...
                                  className="text-[9px] text-muted hover:text-accent font-body underline"
                                >
                                ...
                                <>
                                  <input
                                    ...
                                    className="text-[11px] border border-line px-2 py-1 rounded bg-white focus:outline-none w-full"
                                  />
                                  <div className="flex flex-wrap justify-center gap-1">
                                    <button
                                      ...
                                      className="text-[10px] bg-accent text-white px-2 py-1 rounded hover:bg-accent-deep transition-colors"
                                    >
                                      Kaydet
                                    </button>
                                    <button
                                      ...
                                      className="text-[10px] text-muted px-2 py-1 font-body hover:text-accent transition-colors"
                                    >
                                      Vazgeç
                                    </button>
                                  </div>
        ...
        {activeTab === 'orderRequests' && (
          <div>
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-heading text-[24px] text-ink">Müşteri Talepleri</h2>
              <button
                type="button"
                onClick={() => loadOrderRequests()}
                disabled={orderRequestsLoading}
                className="text-[11px] uppercase tracking-wider font-body px-4 py-2 border border-line hover:border-accent text-ink-soft hover:text-accent transition-colors disabled:opacity-50"
              >
                {orderRequestsLoading ? 'Yükleniyor...' : 'Yenile'}
              </button>
            </div>
            <p className="text-[12px] font-body text-muted mb-4 max-w-2xl leading-relaxed">
            ...
            {orderRequestsLoading && orderRequests.length === 0 ? (
              ...
              <p className="text-muted font-body text-sm">Kayıtlı talep yok.</p>
            ) : orderRequests.length > 0 ? (
              <div className="bg-white overflow-x-auto border border-line/40">
                <table className="w-full text-[12px] font-body">
                  <thead>
                    ...
                    {orderRequests.map((req: any, reqIdx: number) => {
                      ...
                      return (
                        <tr
                          ...
                          className="border-b border-line/30 hover:bg-bg/50 align-top"
                        >
                          ...
                          <td className="px-4 py-3 text-ink-soft break-all max-w-[160px]">
                            {customerLabel}
                          </td>
                          <td className="px-4 py-3">
                            <span
                          ...
                          <td className="px-4 py-3 text-muted whitespace-nowrap">{createdAt}</td>
                            ...
                            {req?.status === 'pending' ? (
                              <div className="flex flex-col sm:flex-row gap-1 justify-center items-stretch">
                                <button
                                  ...
                                  className="text-[10px] bg-accent text-white px-2 py-1.5 rounded hover:bg-accent-deep transition-colors disabled:opacity-50"
                                >
                                  {rowBusy ? '…' : 'Onayla'}
                                </button>
                                <button
                                  ...
                                  className="text-[10px] border border-line text-ink-soft px-2 py-1.5 rounded hover:border-accent transition-colors disabled:opacity-50"
                                >
                                  {rowBusy ? '…' : 'Reddet'}
                                </button>
                              </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-heading text-[24px] text-ink">
                {selectedCategory ? (
                  <><button onClick={() => setSelectedCategory(null)} className="text-accent hover:text-accent-deep mr-2">← </button>{selectedCategory}</>
                ) : showAllProducts ? (
                  <><button onClick={() => setShowAllProducts(false)} className="text-accent hover:text-accent-deep mr-2">← </button>Tüm Ürünler</>
                ) : 'Ürünler'}
              </h2>
              <div className="flex gap-2">
                ...
                <button onClick={() => setShowAddProduct(!showAddProduct)} className="px-4 py-2 bg-accent text-white text-[11px] uppercase tracking-wider hover:bg-accent-deep transition-colors whitespace-nowrap">
            ...
            {showAddProduct && (
              <div className="bg-white p-6 mb-6 space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <label className="flex items-center gap-2 text-[12px] font-body"><input type="checkbox" checked={newProduct.isActive} onChange={(e) => setNewProduct({ ...newProduct, isActive: e.target.checked })} className="accent-accent" /> Aktif</label>
                  <button onClick={saveNewProduct} disabled={!newProduct.title || !newProduct.price} className="px-6 py-2 bg-accent text-white text-[11px] uppercase tracking-wider hover:bg-accent-deep transition-colors disabled:opacity-50">Kaydet</button>
                </div>
              </div>
            )}
            ...
            {!selectedCategory && !showAllProducts && !productSearch && (
              <div className="mb-4">
                <button onClick={() => setShowAllProducts(true)} className="w-full bg-white p-4 text-center border border-line hover:border-accent transition-colors text-[13px] font-body text-accent font-medium">
            ...
            {!selectedCategory && !showAllProducts && !productSearch && (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 mb-6">
                {categoryList.map(([cat, count]) => (
                  <button key={cat} onClick={() => setSelectedCategory(cat)} className="bg-white p-4 text-left hover:border-accent border border-line transition-colors">
                    <p className="text-[13px] font-body font-medium text-ink">{cat}</p>
                    <p className="text-[11px] font-body text-muted mt-1">{count} ürün</p>
                  </button>
                ))}
              </div>
            )}
            ...
            {(selectedCategory || showAllProducts || productSearch) && filteredProducts.length > 0 && (
              <div className="bg-white overflow-x-auto">
                <table className="w-full text-[11px] font-body">
                  <thead><tr className="border-b border-line text-muted uppercase tracking-wider text-[9px]">
                    <th className="text-left px-3 py-2 cursor-pointer hover:text-accent" onClick={() => toggleSort('display_title')}>Ürün{sortIcon('display_title')}</th>
                    <th className="text-left px-3 py-2 cursor-pointer hover:text-accent" onClick={() => toggleSort('trendyol_barcode')}>Barkod{sortIcon('trendyol_barcode')}</th>
                    <th className="text-right px-3 py-2 cursor-pointer hover:text-accent" onClick={() => toggleSort('price')}>Fiyat{sortIcon('price')}</th>
                    <th className="text-left px-3 py-2">Not</th>
                    <th className="text-center px-3 py-2 cursor-pointer hover:text-accent" onClick={() => toggleSort('trendyol_stock')}>Stok{sortIcon('trendyol_stock')}</th>
                    <th className="text-center px-3 py-2 cursor-pointer hover:text-accent" onClick={() => toggleSort('sales_count')}>Satış{sortIcon('sales_count')}</th>
                    <th className="text-center px-3 py-2">Durum</th>
                    <th className="text-center px-3 py-2">İşlem</th>
                  </tr></thead>
                  <tbody>{filteredProducts.map((p: any) => (
                    <tr key={p.id} className="border-b border-line/30 hover:bg-bg/50">
                      <td className="px-3 py-2"><div className="flex items-center gap-2">
                        <div className="w-8 h-10 bg-surface-muted shrink-0 overflow-hidden">{p.display_images?.[0] && <img src={p.display_images[0]} alt="" className="w-full h-full object-cover" />}</div>
                        <span className="font-medium truncate max-w-[160px]">{p.display_title}</span>
                      </div></td>
                      <td className="px-3 py-2 text-muted text-[10px]">{p.trendyol_barcode || '-'}</td>
                      <td className="px-3 py-2 text-right">
                        {editingCell?.id === p.id && editingCell?.field === 'custom_price' ? (
                          <input type="number" autoFocus value={editingValue} onChange={(e) => setEditingValue(e.target.value)}
                            ...
                            className="w-20 text-right border border-accent px-1 py-0.5 text-[11px] focus:outline-none" />
                        ) : (
                          <span className={`cursor-pointer hover:text-accent ${p.custom_price ? 'text-accent font-medium' : 'text-ink'}`}
                      ...
                      <td className="px-3 py-2">
                        {editingCell?.id === p.id && editingCell?.field === 'note' ? (
                          <input type="text" autoFocus value={editingValue} onChange={(e) => setEditingValue(e.target.value)}
                            ...
                            className="w-24 border border-accent px-1 py-0.5 text-[11px] focus:outline-none" />
                        ) : (
                          <span className="cursor-pointer text-muted hover:text-accent text-[10px] truncate block max-w-[100px]"
                      ...
                      <td className="px-3 py-2 text-center text-muted">{p.sales_count || 0}</td>
                      ...
                      <td className="px-3 py-2 text-center"><button onClick={() => setEditingProduct({ ...p })} className="text-[10px] text-accent hover:text-accent-deep transition-colors uppercase tracking-wider">Düzenle</button></td>
                    </tr>
                  ))}</tbody>
                </table>
              </div>
            )}
            {(selectedCategory || showAllProducts || productSearch) && filteredProducts.length === 0 && <p className="text-muted font-body text-sm">Ürün bulunamadı.</p>}
              ...
              <div className="fixed inset-0 bg-ink/50 z-50 flex items-center justify-center p-4">
                <div className="bg-white w-full max-w-md p-6 space-y-4">
                  ...
                  <div className="flex gap-3 pt-2">
                    <button onClick={saveProductOverride} className="flex-1 py-2 bg-accent text-white text-[11px] uppercase tracking-wider hover:bg-accent-deep transition-colors">Kaydet</button>
                    <button onClick={() => setEditingProduct(null)} className="flex-1 py-2 border border-line text-muted text-[11px] uppercase tracking-wider hover:border-accent transition-colors">İptal</button>
                  </div>
                </div>
              </div>
            )}
        ...
        {activeTab === 'campaigns' && (
          <div>
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-heading text-[24px] text-ink">Kampanyalar</h2>
              <button onClick={() => { setShowCampaignForm(!showCampaignForm); if (showCampaignForm) { setEditingCampaignId(null); setCampaignForm(emptyCampaignForm) } }} className="px-4 py-2 bg-accent text-white text-[11px] uppercase tracking-wider hover:bg-accent-deep transition-colors">{showCampaignForm ? 'İptal' : '+ Yeni Kampanya'}</button>
            </div>
            {showCampaignForm && (
              <div className="bg-white p-6 mb-6 space-y-4">
                ...
                {campaignForm.type === 'banner' && (<><input className={inputClass} placeholder="Banner Metni" value={campaignForm.banner_text} onChange={(e) => setCampaignForm({ ...campaignForm, banner_text: e.target.value })} /><div className="flex items-center gap-3"><label className="text-[12px] font-body text-muted">Renk:</label><input type="color" value={campaignForm.banner_color} onChange={(e) => setCampaignForm({ ...campaignForm, banner_color: e.target.value })} className="w-10 h-8 border-0 cursor-pointer" /></div></>)}
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="text-[10px] font-body text-muted uppercase tracking-wider">Başlangıç</label><input className={inputClass} type="datetime-local" value={campaignForm.starts_at} onChange={(e) => setCampaignForm({ ...campaignForm, starts_at: e.target.value })} /></div>
                  <div><label className="text-[10px] font-body text-muted uppercase tracking-wider">Bitiş</label><input className={inputClass} type="datetime-local" value={campaignForm.ends_at} onChange={(e) => setCampaignForm({ ...campaignForm, ends_at: e.target.value })} /></div>
                </div>
                <button onClick={saveCampaign} disabled={!campaignForm.name} className="w-full py-2.5 bg-accent text-white text-[11px] uppercase tracking-wider hover:bg-accent-deep transition-colors disabled:opacity-50">{editingCampaignId ? 'Güncelle' : 'Kampanya Oluştur'}</button>
              </div>
            )}
            <div className="space-y-3">{localCampaigns.length === 0 ? <p className="text-muted font-body text-sm">Henüz kampanya yok.</p> : localCampaigns.map((c: any) => (
              <div key={c.id} className="bg-white p-4 flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2"><span className="font-body text-[13px] font-medium text-ink">{c.name}</span><span className="text-[9px] bg-surface-muted px-1.5 py-0.5 rounded text-muted uppercase">{c.type.replace('_', ' ')}</span>{!c.is_active && <span className="text-[9px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded">Pasif</span>}</div>
                  <div className="text-[11px] text-muted font-body mt-1 space-x-3">{c.code && <span>Kod: <strong>{c.code}</strong></span>}{c.discount_value && <span>{c.discount_type === 'percent' ? `%${c.discount_value}` : formatPrice(c.discount_value)}</span>}{c.banner_text && <span>{c.banner_text}</span>}{c.max_uses && <span>Kullanım: {c.used_count}/{c.max_uses}</span>}</div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => startEditCampaign(c)} className="text-[10px] text-accent hover:text-accent-deep transition-colors px-2 py-1">Düzenle</button>
        ...
        {activeTab === 'reviews' && (
          <div>
            <h2 className="font-heading text-[24px] text-ink mb-6">Yorumlar</h2>
            {localReviews.length === 0 ? <p className="text-muted font-body text-sm">Henüz yorum yok.</p> : (
                ...
                <div key={review.id} className="bg-white p-4 flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <div className="flex gap-0.5">{[1,2,3,4,5].map((s) => <svg key={s} className={`w-3 h-3 ${s <= review.rating ? 'fill-accent' : 'fill-line'}`} viewBox="0 0 24 24"><path d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.562.562 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" /></svg>)}</div>
                      <span className="text-[11px] text-muted font-body">{review.guest_name || 'Üye'} · {new Date(review.created_at).toLocaleDateString('tr-TR')}</span>
                      {!review.is_approved && <span className="text-[9px] bg-yellow-100 text-yellow-800 px-1.5 py-0.5 rounded">Bekliyor</span>}
                    </div>
                    {review.title && <p className="text-[12px] font-body font-medium text-ink">{review.title}</p>}
                    <p className="text-[12px] font-body text-ink-soft truncate">{review.body}</p>
                  </div>
        ...
        {activeTab === 'blog' && (
          <div>
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-heading text-[24px] text-ink">Blog Yazıları</h2>
              <button
                onClick={() => setBlogForm({ ...emptyBlogForm })}
                className="px-4 py-2 bg-accent text-white text-[11px] uppercase tracking-wider hover:bg-accent-deep transition-colors"
              >
                + Yeni Yazı
              </button>
            </div>
            ...
            {!blogLoaded ? (
              ...
              <p className="text-muted font-body text-sm">Henüz yazı yok.</p>
            ) : (
              <div className="bg-white overflow-x-auto">
                <table className="w-full text-[12px] font-body">
                  <thead>
                    <tr className="border-b border-line bg-bg text-muted uppercase tracking-wider text-[10px]">
                  ...
                  <tbody>
                    {blogPosts.map((post: any) => (
                      <tr key={post.id} className="border-b border-line/30 hover:bg-bg/50">
                        <td className="px-4 py-3 text-ink max-w-[240px] truncate">{post.title}</td>
                        <td className="px-4 py-3 text-muted max-w-[160px] truncate">{post.slug}</td>
                        <td className="px-4 py-3 text-center">
                          {post.published
                        ...
                        <td className="px-4 py-3 text-center flex items-center justify-center gap-3">
                          <button
                            ...
                            className="text-[10px] text-accent hover:text-accent-deep transition-colors uppercase tracking-wider"
                          >
                            Düzenle
                          </button>
                          <button
                ...
                <div className="bg-white w-full max-w-2xl p-6 space-y-4 my-8">
                  ...
                  <div className="flex gap-3 pt-2">
                    <button
                      ...
                      disabled={blogSaving}
                      className="flex-1 py-2 bg-accent text-white text-[11px] uppercase tracking-wider hover:bg-accent-deep transition-colors disabled:opacity-50"
                    >
                      {blogSaving ? 'Kaydediliyor...' : 'Kaydet'}
                    </button>
                    <button
                      onClick={() => setBlogForm(null)}
                      className="flex-1 py-2 border border-line text-muted text-[11px] uppercase tracking-wider hover:border-accent transition-colors"
                    >
                      İptal
                    </button>
                  </div>
        ...
        {activeTab === 'sync' && (
          <div>
            ...
            <h3 className="font-heading text-[18px] text-ink mb-4">Son Sync Logları</h3>
            {syncLogs.length === 0 ? <p className="text-muted font-body text-sm">Henüz sync yapılmamış.</p> : (
              <div className="bg-white">{syncLogs.map((log: any) => (
                <div key={log.id} className="px-4 py-3 border-b border-line/30 flex items-center justify-between text-[12px] font-body">
                  <span className="text-muted">{new Date(log.synced_at).toLocaleString('tr-TR')}</span>
